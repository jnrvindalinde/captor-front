...
import type { Resource } from "@/app/resources/_data";
...
/** Live story from the DB (overrides i18n keyed lookups when provided). */
...
/** Hardcoded fallback (used when API returns no live stories). */
const fallbackStories: StoryEntry[] = [
...
/* Hardcoded fallback latest blog post (used when API returns nothing). */
// eslint-disable-next-line @typescript-eslint/no-unused-vars
const fallbackLatestPost: HomeLatestPost = {
...
/* ========================== STORIES CAROUSEL ========================== */
function StoriesCarousel({ liveStories }: { liveStories: HomeStory[] | null }) {
  const reduce = useReducedMotion();
  const t = useTranslations("home.stories");
  const tc = useTranslations("home.storiesCarousel");
  const pageSize = 3;
  const useLive = liveStories !== null && liveStories.length > 0;
  const fallbackList = fallbackStories;
  const liveList = liveStories ?? [];
  const total = useLive ? liveList.length : fallbackList.length;
  ...
  return (
    <div className="stories-carousel">
      <AnimatePresence mode="wait">
        ...
        >
          {useLive
            ? liveList.slice(start, start + pageSize).map((s) => (
                  ...
                  >
                    ...
                    <div className="story-card__person">
                      <div className="story-card__avatar" aria-hidden="true">
                        {s.image ? <img src={s.image} alt={s.name} /> : s.name.slice(0, 2).toUpperCase()}
                      </div>
            ...
            : fallbackList.slice(start, start + pageSize).map((s) => (
...
export type HomeClientProps = {
  liveStories: HomeStory[] | null;
  latestPost: HomeLatestPost;
  previewResources: Resource[];
};

export function HomeClient({ liveStories, latestPost, previewResources }: HomeClientProps) {
  const reduce = useReducedMotion();
  ...
  return (
    <main>
      ...
      <Section id="stories">
        ...
        <StoriesCarousel liveStories={liveStories} />
      </Section>
      ...
      <Section id="resources">
        ...
        <motion.div
          ...
          viewport={{ once: true, amount: 0.15 }}
        >
          {previewResources.slice(0, 3).map((r) => (
            <motion.article
              key={r.title}
        ...
        >
          ...
          <div className="blog-latest__body">
            ...
            <div className="blog-latest__cta">
              <ArrowLink href={`/blog/${latestPost.slug}`}>{t("blogSection.readPost")}</ArrowLink>
            </div>
          </div>
        </motion.article>
      </Section>
