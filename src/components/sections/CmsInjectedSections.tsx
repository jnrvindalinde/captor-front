...
/**
 * Client-only renderer for CMS pages. Fetches /api/public/pages/{slug} in
 * useEffect and renders the section list using the same visual primitives as
 * the server-side `PageRenderer`. Used in client components (e.g. the home
 * page) to inject editable sections above or below the hardcoded design.
 *
 * Returns null while loading or when the page has no published sections, so
 * it never reserves layout space until there is something to show.
 */
...
function MarqueeLogos({ data }: { data: Record<string, unknown> }) {
  const slug = (typeof data.collection_slug === "string" && data.collection_slug) || "partners";
  return <CmsPartnersMarquee defaultItems={[]} slug={slug} />;
}
...
const MAP: Record<string, (p: { data: Record<string, unknown>; locale: Locale }) => React.ReactNode> = {
  "hero.split": HeroSplit,
  "hero.centered": HeroCentered,
  richtext: RichText,
  "cta.banner": CtaBanner,
  "marquee.logos": ({ data }) => <MarqueeLogos data={data} />,
...
export function CmsInjectedSections({ slug }: { slug: string }) {
  ...
  useEffect(() => {
    ...
    if (!base) return;
    let cancelled = false;
    fetch(`${base.replace(/\/$/, "")}/api/public/pages/${encodeURIComponent(slug)}`)
      .then((r) => (r.ok ? r.json() : null))
      .then((json) => {
        if (cancelled || !json?.data) return;
        setPage(json.data as FetchedPage);
      })
      .catch(() => { /* page not found / unreachable — render nothing */ });
  ...
  return (
    <div className="cms-injected">
      {sorted.map((s) => {
        const Cmp = MAP[s.type];
        return (
          <div key={s.uuid}>
            {Cmp ? <Cmp data={s.data ?? {}} locale={locale} /> : null}
          </div>
        );
      })}
    </div>
